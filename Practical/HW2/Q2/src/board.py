...
class Board:
    ...
    def custome_place_piece_imaginary(self, move, player_number):
        """ Given the player number, place a piece with its color in position (i, j).
                Upon success, return the count of overturned pieces plus one. Return 0 otherwise."""
        origin, destination = move
        black_move = player_number == self.black
        ...
        return self.custome_get_scores(self.imaginary_board_grid), self.is_game_over(self.imaginary_board_grid)

    def custome_get_scores(self, board):
        """ Return the scores of the game for both players."""
        scores = {0: 0, 1: 0}
        for row in range(self.__n):
            for col in range(self.__n):
                if board[row][col] == self.black:
                    scores[self.black] += 1 + (np.e ** row) * row
                elif board[row][col] == self.white:
                    scores[self.white] += 1 + (np.e ** (self.__n - row - 1)) * (self.__n - row - 1)
        scores[self.black] += self.winning_score(board, self.black)
        scores[self.white] += self.winning_score(board, self.white)
        scores[self.black], scores[self.white] = scores[self.black] - scores[self.white], scores[self.white] - scores[self.black]
        return scores
